...
class Reserva:
    ...
    def to_dict(self):
         return{"id": self.id, "recurso": self.recurso, "morado_id": self.morador_id, "data_inicio": self.data_inicio,
                "data_fim": self.data_fim, "status": self.status}
    
    @staticmethod
    def from_dict(data):
         return Reserva(reserva_id=data.get("id"), recurso=data.get("recurso"), morador_id=data.get("morador_id"), 
                        data_inicio = data.get("data_inicio"), data_fim = data.get("data_fim"), status= data.get("status", StatusReserva.CONFIRMADA.value))
...
class ReservaModel:
     ...
     def add_reserva(self, reserva: Reserva):
         if self.verificar_disponibildiade(reserva.recurso, reserva.data_inicio, reserva.data_fim):
     ...
     def verificar_disponibildiade(self, recurso: str, inicio_str: str, fim_str: str):
         formato= "%Y-%m-%d %H:%M"
         try:
